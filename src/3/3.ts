...
// convert directions into co-ordinates array e.g.
// R8,U5,L5,D3 becomes
// [ [0,0], [1,0], [2,0], [3,0], ... , [3,3], [3,2] ]
export const convertToXY = (directions: string): number[][] => {
  let directionsArray: string[] = directions.split(',');
  let currentXY: number[] = [0,0];
  let coords: number[][] = [[...currentXY]];
  for (let d = 0; d < directionsArray.length; d++) {
    let direction: string = directionsArray[d].substring(0,1);
    let distance: number = parseInt(directionsArray[d].substring(1),10);
    for (let i = 0; i < distance; i++) {
      ...
      coords.push([...currentXY]);
    }
  }
  return(coords);
}

// let a = convertToXY('R8,U5,L5,D3');
// let b = convertToXY('U7,R6,D4,L4');

// console.log(a);
// console.log(b);

export const findIntersections = (pathA: number[][], pathB: number[][]): number[][] => {
  const intersections: number[][] = [];
  for (let i = 0; i < pathA.length; i++) {
    let a: number[] = pathA[i];
    for (let j = 0; j < pathB.length; j++) {
      let b: number[] = pathB[j];
      if (a[0] === b[0] && a[1] === b[1]) {
        intersections.push(a);
      }
    }
  }
  console.log('Found %s intersections', intersections.length);
  console.log('Intersections', intersections);
  return intersections;
}

// export const findIntersections2 = (pathA: number[][], pathB: number[][]): number[][] => {
//   const intersections: number[][] = intersectionWith(pathA, pathB, isEqual);
//   console.log('Found %s intersections', intersections.length);
//   console.log('Intersections', intersections);
//   return intersections;
// }
...
export const calculateDistanceToNearestIntersection = (intersections: number[][]): number => {
  if (intersections.length < 1) {
    return 0;
  }
  let lowestDistance: number = Math.abs(intersections[1][0]) + Math.abs(intersections[1][1]);
  if (intersections.length === 1) {
    return lowestDistance;
  }
  for (let k = 2; k < intersections.length; k++) {
    let distance: number = Math.abs(intersections[k][0]) + Math.abs(intersections[k][1]);
...
// const distance = calculateDistanceToNearestIntersection(intersections);

// console.log(distance);

export const getLowestDistanceToIntersection = (intersections: number[][], pathA: number[][], pathB: number[][]): number => {
  let lowestDistance = 0;
  for (let i = 1; i < intersections.length; i++) { // skip first point (0, 0)
    let distance = 0;

    // how far along path A is this point
    for (let a = 1; a < pathA.length; a++) { 
      if (pathA[a][0] === intersections[i][0] && pathA[a][1] === intersections[i][1]) {
        distance = distance + a;
        // console.log('Distance on path A for %s is %s', intersections[i], a);
        break;
      }
    }
    
    // how far along path B is this point
    for (let b = 1; b < pathB.length; b++) { 
      if (pathB[b][0] === intersections[i][0] && pathB[b][1] === intersections[i][1]) {
        distance = distance + b;
        // console.log('Distance on path B for %s is %s', intersections[i], b);
        break;
      }
    }
...
export const findFewestStepsToPort = (directionsA: string, directionsB: string): number => {
  ...
  return getLowestDistanceToIntersection(intersections, a, b);

  // check each intersection in path A and path B to find the first intersection

  // const distancePathA = getDistanceToFirstIntersection(a, intersections);
  // const distancePathB = getDistanceToFirstIntersection(b, intersections);
  // console.log('Distance on path a is %s', distancePathA);
  // console.log('Distance on path b is %s', distancePathB);


  // return distancePathA + distancePathB;
}